...
from src.nnunet.experiment_planning.common_utils import split_4d_nifti
from src.nnunet.paths import nnUNet_raw_data, nnUNet_cropped_data, preprocessing_output_dir
from src.nnunet.preprocessing.cropping import ImageCropper
...
def create_lists_from_splitted_dataset(base_folder_splitted):
    """create lists form split dataset"""
    lists = []
...
def get_caseIDs_from_splitted_dataset_folder(folder):
    """get caseIDs from split dataset folder"""
...
def crop(task_string, override=False, num_threads=default_num_threads, nnUNet_cropped_data=None, nnUNet_raw_data=None):
    """crop function"""
...
def plan_and_preprocess(task_string, processes_lowres=default_num_threads, processes_fullres=3, no_preprocessing=False):
    ...
    if not no_preprocessing:
        ...
        # if there is more than one my_data_identifier (different branches) then this code will run for all of them if
