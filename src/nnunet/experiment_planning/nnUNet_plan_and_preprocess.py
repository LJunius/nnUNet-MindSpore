...
from src import nnunet
from src.nnunet.experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from src.nnunet.experiment_planning.utils import crop
from src.nnunet.paths import join, maybe_mkdir_p, os, nnUNet_raw_data, nnUNet_cropped_data, preprocessing_output_dir
from src.nnunet.preprocessing.sanity_checks import verify_dataset_integrity
...
def main():
    ...
    parser.add_argument("-t", "--task_ids", nargs="+", default=['40'], help="List of integers belonging to the task ids you wish to run"
    ...
    parser.add_argument("-pl3d", "--planner3d", type=str, default="ExperimentPlanner3D_v21",
                        help="Name of the ExperimentPlanner class for the full resolution 3D U-Net and U-Net cascade. "
                             "Default is ExperimentPlanner3D_v21. Can be 'None', "
                             "in which case these U-Nets will not be "
                             "configured")
    parser.add_argument("-pl2d", "--planner2d", type=str, default="ExperimentPlanner2D_v21",
                        help="Name of the ExperimentPlanner class for the 2D U-Net. "
    ...
    parser.add_argument("-overwrite_plans", type=str, default=None, required=False,
                        help="Use this to specify a plans file that should be used instead of whatever nnU-Net would "
                             ...
                             "dataset is the same as on the source dataset "
                             "and the weights can therefore be transferred.\n"
    ...
    if planner_name3d is not None:
        planner_3d = recursive_find_python_class([search_in], planner_name3d,
                                                 current_module="src.nnunet.experiment_planning")
    ...
    task_loop_logic(args, dont_run_preprocessing, planner_2d, planner_3d, tasks, tf, tl)


def task_loop_logic(args, dont_run_preprocessing, planner_2d, planner_3d, tasks, tf, tl):
    ...
    for t in tasks:
        ...
        collect_intensityproperties = bool(("CT" in modalities) or ("ct" in modalities))
