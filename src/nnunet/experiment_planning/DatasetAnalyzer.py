...
import numpy as np
from batchgenerators.utilities.file_and_folder_operations import isfile, join, save_pickle, load_pickle, pickle, load_json
from skimage.morphology import label

from src.nnunet.configuration import default_num_threads
from src.nnunet.preprocessing.cropping import get_patient_identifiers_from_cropped_files
...
class DatasetAnalyzer():
    ...
    def _check_if_all_in_one_region(seg, regions):
        ...
        for r in regions:
            ...
            for c in r:
                new_seg[seg == c] = 1
            _, numlabels = label(new_seg, return_num=True)
            if numlabels != 1:
                res[tuple(r)] = False
            else:
                res[tuple(r)] = True
    ...
    def _collect_class_and_region_sizes(seg, all_classes, vol_per_voxel):
        """collect class region information"""
