...
from batchgenerators.utilities.file_and_folder_operations import os, isfile, subfiles, join, load_pickle
...
def get_case_identifiers_from_raw_folder(folder):
    """get case identifier from raw folder"""
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 ...
                 pad_sides=None):
        """
        This is the basic data loader for 3D networks.
        ...
        If there are only npz files present in that folder, the data loader will unpack them on the fly.
        This may take a while and increase CPU usage.
        Therefore, I advise you to call unpack_dataset(folder) first, which will unpack all npz to npy.
        Don't forget to call delete_npy(folder) after you are done with training?
        Why all the hassle? Well the decathlon dataset is huge.
        Using npy for everything will consume >1 TB and that is uncool
        given that I (Fabian) will have to store that permanently on /datasets and my local computer.
        With this strategy all data is stored in a compressed format (factor 10 smaller)
        and only unpacked when needed.
        :param data: get this with load_dataset(folder, stage=0).
        Plug the return value in here and you are g2g (good to go)
        :param patch_size: what patch size will this data loader return?
        it is common practice to first load larger
        patches so that a central crop after data augmentation can be done to reduce border artifacts.
        If unsure, use get_patch_size() from data_augmentation.default_data_augmentation
        :param final_patch_size: what will the patch finally be cropped to (after data augmentation)?
        this is the patch size that goes into your network.
        We need this here because we will pad patients in here so that patches at the
        ...
        :param random: Sample keys randomly; CAREFUL! non-random sampling requires batch_size=1,
         otherwise you will iterate batch_size times over the dataset
        :param oversample_foreground: half the batch will be forced to contain at least some foreground
        (equal prob for each of the foreground classes)
        """
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            force_fg = bool(self.get_do_oversample(j))
            ...
            lb_x, lb_y, lb_z, shape, ub_x, ub_y, ub_z = self.generate_train_batch_bound(case_all_data, need_to_pad)
    ...
    def generate_train_batch_bound(self, case_all_data, need_to_pad):
        """This is part of generate train batch code"""
        ...
        return lb_x, lb_y, lb_z, shape, ub_x, ub_y, ub_z
...
class DataLoader2D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
        ...
        If there are only npz files present in that folder, the data loader will unpack them on the fly.
        This may take a while and increase CPU usage.
        Therefore, I advise you to call unpack_dataset(folder) first, which will unpack all npz
        to npy.
        Don't forget to call delete_npy(folder) after you are done with training?
        Why all the hassle? Well the decathlon dataset is huge.
        Using npy for everything will consume >1 TB and that is uncool
        given that I (Fabian) will have to store that permanently on /datasets and my local computer.
        With htis strategy all data is stored in a compressed format (factor 10 smaller) and only unpacked when needed.
        :param data: get this with load_dataset(folder, stage=0).
        Plug the return value in here and you are g2g (good to go)
        ...
        :param random: sample randomly; CAREFUL! non-random sampling requires batch_size=1,
        otherwise you will iterate batch_size times over the dataset
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            bbox_x_ub, bbox_y_ub, valid_bbox_x_lb, \
            valid_bbox_x_ub, valid_bbox_y_lb, valid_bbox_y_ub = self.generate_train_batch_part(
                bbox_x_lb, bbox_y_lb, shape)
    ...
    def generate_train_batch_part(self, bbox_x_lb, bbox_y_lb, shape):
        """This function is part of generate train batch methed 2d"""
        bbox_x_ub = bbox_x_lb + self.patch_size[0]
        bbox_y_ub = bbox_y_lb + self.patch_size[1]
        # whoever wrote this knew what he was doing (hint: it was me). We first crop the data to the region of the
        # bbox that actually lies within the data. This will result in a smaller array which is then faster to pad.
        ...
        valid_bbox_y_lb = max(0, bbox_y_lb)
        valid_bbox_y_ub = min(shape[1], bbox_y_ub)
        return bbox_x_ub, bbox_y_ub, valid_bbox_x_lb, valid_bbox_x_ub, valid_bbox_y_lb, valid_bbox_y_ub
